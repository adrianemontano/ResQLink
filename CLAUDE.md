...
- **Do not spawn sub-agents** for tasks in this repository. Do the work
  directly yourself rather than delegating to the Agent tool.
- **Do not run lint, test, or build commands** (e.g. `vendor/bin/pint`,
  ...
  me first.** Ask before running any of these, even if a change would
  normally warrant it.
- **Exception (suggest, don't auto-run):** when a change involves a big
  ...
  and wait for my "yes" before running them. Still do not run them on your
  own.
